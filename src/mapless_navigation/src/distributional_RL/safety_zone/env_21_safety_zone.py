...
class Env():
	...
	def getState(self, scan):
		...
		relationship = [round(heading,3), round(current_distance/20,2), round(obstacle_min_range/10.0,2)]
	...
	def setReward(self, state, done, action):
		...
		if not done: 
			...
			if distance_difference <= 0.0:
				...
				distance_difference += 0.2*distance_difference
			else:
			...
			# 	reward += -10
			if min(safety_zone) < (0.8- self.min_range)/(self.max_range-self.min_range):
				# Ct = 0.02
				self.safety_time +=1 
				print("self.safety_time",self.safety_time)
				x = 0.05*(exp(0.16*self.safety_time))
				# a = min(safety_zone)
				Ct = np.array([x])
				print("unsafe")
			else: 
				self.safety_time =0 
			if self.safety_time >=19:
				self.safety_time = 19
			if obstacle_min_range < 1.0:
				reward += -30*(1-(obstacle_min_range/1.0))
			
			delta_v = abs(action[1] - self.v_after)
			delta_w = abs(action[0] - self.w_after)
			if  delta_v >= 0.3:
				reward += -2*(abs(action[0] - self.v_after))
			if abs(action[0] - self.w_after) > 0.3:
				reward += -1*(abs(action[0] - self.v_after))
			print("delta_w_reward",delta_v,delta_w )
	...
	def step(self, action):
		...
		reward,Ct = self.setReward(state, done, action)
		self.delta_time = time.time()-start_time
		print("1 step: {}", self.delta_time)	
		self._set_action(action,state)
		self.v_after = action[1]
		self.w_after = action[0]	
		return np.asarray(state), reward, done, arrival, Ct 
	...
	def _set_action(self, action,state):
		...
		# print("linear_speed,angular_speed",linear_speed,angular_speed)
		# self.v_after = linear_speed
		# self.w_after = angular_speed

		self.time_step += 1
		linear_speed = action[1]
		angular_speed = action[0]
		# We tell TurtleBot2 the linear and angular speed to set to execute
		# if min(state[9:11]) < (0.5- self.min_range)/(self.max_range-self.min_range):
		# 	self.move_base(0.0, angular_speed, epsilon=0.05, update_rate=10)
		# 	print("stop velocity")
		# else:
