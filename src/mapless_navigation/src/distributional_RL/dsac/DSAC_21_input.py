...
wandb.init(name='21_input_DSAC_1013', project="Rastech_Soft_Actor_critic_nana_robot",config=config)
import GPUtil
import psutil
...
class PolicyNetwork(nn.Module):
    ...
    def sample(self, state, epsilon):
        ...
        wandb.log({"epsilon decay": epsilon})    
...
class SAC_Agent:
    def __init__(
        ...
        self.policy  = PolicyNetwork(self.state_dim, self.action_dim, self.lr_pi, self.DEVICE)
        self.target_policy = PolicyNetwork(self.state_dim, self.action_dim, self.lr_pi, self.DEVICE)
        # wandb.watch(self.PI)
    ...
    def get_tau(self, obs, actions, fp=None):
        ...
        tau = torch.cumsum(presum_tau, dim=1)  # (N, T), note that they are tau1...tauN in the paper, cumulative sum of element values 
        with torch.no_grad():
    ...
    def choose_action(self, s):
        with torch.no_grad():
            action, _ , _ , log_prob, _ = self.policy.sample(s.to(self.DEVICE), self.epsilon)
    ...
    def train_agent(self,ep):
        ...
        obs, actions, rewards,next_obs, terminals = mini_batch
        # epsilon = 1
        ...
        # if collision_rate < 0.1 and self.epsilon >=0.25:
        #     self.epsilon = self.epsilon *0.9999 
        
        """
        Update Alpha
        """
        new_actions, policy_mean, policy_log_std, log_pi, _ = self.policy.sample(obs,self.epsilon)
        # print("log_pi", log_pi)
        ...
        """
        with torch.no_grad():
            new_next_actions, _, _, new_log_pi, _ = self.target_policy.sample(next_obs, self.epsilon)
            ...
            min_target_z = torch.min(target_z1_values, target_z2_values)
            target_z_values = torch.stack([(min_target_z[:,:,i] - alpha * new_log_pi) for i in range(0,32)])
            ...
            z_target = self.reward_scale * rewards + terminals * self.discount * target_z_values
            wandb.log({"z_target": z_target})  
...
if __name__ == '__main__':
    ...
    for EP in range(EPISODE):   
        ...
        state_prime, reward, done, arrival = env.step(action)
